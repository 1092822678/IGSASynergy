...
from torch_geometric.nn import PNAConv, GCNConv, global_mean_pool
...
# === 细胞系子图构建 ===
# def get_cell_line_subgraph(cell_line_id, cpi, ppi, protein_to_idx):
...
#     # 直接使用全局 `protein_to_idx` 映射
#     edge_index = torch.tensor([
#         [protein_to_idx[p1], protein_to_idx[p2]] for p1, p2 in zip(sub_ppi['protein_a'], sub_ppi['protein_b'])
#     ], dtype=torch.long).t()

#     num_nodes = len(related_proteins)
#     if num_nodes == 0:
#         return None  # 处理极端情况，防止空子图

#     x = torch.tensor([protein_to_idx[p] for p in related_proteins], dtype=torch.long)
    
#     return Data(x=x, edge_index=edge_index)


def get_cell_line_subgraph(cell_line_id, cpi, ppi, protein_to_idx):
    related_proteins = list(set(cpi[cpi['cell'] == cell_line_id]['protein'].tolist()))
    sub_ppi = ppi[(ppi['protein_a'].isin(related_proteins)) & (ppi['protein_b'].isin(related_proteins))]

    if len(related_proteins) == 0:
        return None  # 避免空子图

    local_idx_map = {protein: i for i, protein in enumerate(related_proteins)}
    edge_index = torch.tensor([
        [local_idx_map[p1], local_idx_map[p2]] 
        for p1, p2 in zip(sub_ppi['protein_a'], sub_ppi['protein_b'])
    ], dtype=torch.long).t()

    # `x` 应该是 protein_to_idx 映射的特征，而不是索引
    x = torch.tensor([protein_to_idx[p] for p in related_proteins], dtype=torch.long)
...
class DrugSynergyPredictor(nn.Module):
    def __init__(self, input_dim):
        ...
        self.layer1 = nn.Sequential(nn.Linear(320, 128), nn.BatchNorm1d(128),
    ...
    def forward(self, drug1_feat, drug2_feat, cell_feat, gene_exp):
        combined = torch.cat((drug1_feat, drug2_feat, cell_feat), dim=1)
        # combined = torch.cat((drug1_feat, drug2_feat, cell_feat, gene_exp), dim=1)
...
class ExpSynergy(nn.Module):
    def __init__(self, drug1_gsat, drug2_gsat, cell_gsat, predictor):
        ...
        self.gene_encoder = nn.Sequential(
            nn.Linear(19153, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Linear(128, 64)
        )

    def forward(self, drug1, drug2, cell, gene_exp, epoch, training=True):
        ...
        cell_feat = self.cell_gsat(cell)
        gene_exp = self.gene_encoder(gene_exp)
        
        return self.predictor(drug1_feat, drug2_feat, cell_feat, gene_exp)
        # return self.predictor(drug1_feat, drug2_feat, cell_feat)
        

class CellGCN(nn.Module):
    def __init__(self, num_proteins, input_dim=64, hidden_dim=128, output_dim=64):
        super(CellGCN, self).__init__()
        self.num_proteins = num_proteins
        self.conv1 = GCNConv(num_proteins, hidden_dim)  # 输入变成 num_proteins 维度
    ...
    def forward(self, data):
        ...
        x = F.one_hot(data.x, num_classes=self.num_proteins).float()  # 直接转换为 one-hot
...
def main():
    ...
    gene_express = pd.read_csv("./data/Expression_Public_24Q4_subsetted.csv")
    # cell_line_display_name
    unique_cells = synergy['cell'].unique()
    
    # 全局蛋白质索引字典
    all_proteins = set(ppi['protein_a']).union(set(ppi['protein_b']))
    protein_to_idx = {protein: idx for idx, protein in enumerate(sorted(all_proteins))}
    num_proteins = len(protein_to_idx)  # 计算蛋白质总数

    cell_dict = {cell: index for index, cell in enumerate(unique_cells, start=1)}

    gene_express = gene_express[gene_express['cell_line_display_name'].isin(unique_cells)]
    # 直接删除指定列
    gene_express = gene_express.drop(columns=['depmap_id', 'lineage_1', 'lineage_2', 'lineage_3', 'lineage_6', 'lineage_4'])
    ...
    for _, row in train_df.iterrows():
        ...
        cell_data = get_cell_line_subgraph(row['cell'], cpi, ppi, protein_to_idx)

        # cell_data = cell_dict[row['cell']]
        ...
        gene = gene_express[gene_express['cell_line_display_name']==row['cell']]
        train_set.append((drug_data1, drug_data2, cell_data, synergy_label,
                          torch.tensor(gene.iloc[:,1:].to_numpy(), dtype=torch.float).reshape(-1)))
    ...
    num_nodes = len(set(ppi['protein_a']) | set(ppi['protein_b']))  # 计算唯一蛋白质节点数
    cell_gcn = CellGCN(num_proteins=num_proteins)

    # 初始化模型
    model = ExpSynergy(drug_gsat1, drug_gsat2, cell_gcn, DrugSynergyPredictor(hidden_size)).to(device)
    ...
    for epoch in range(200):
        ...
        for batch in train_loader:
            drug_data1_batch, drug_data2_batch, cell_data_batch, labels_batch, gene_batch = batch
            ...
            optimizer.zero_grad()
            pred = model(drug_data1_batch, drug_data2_batch, cell_data_batch, gene_batch, epoch)
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                pred = model(drug_data1_batch, drug_data2_batch, cell_data_batch, gene_batch, epoch, training=False)
